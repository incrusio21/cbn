...
def get_columns(filters):
	...
	columns = [
		...
		_("Batch") + ":Link/Batch Manufacture:100",
		_("Expired Date") + ":Date:100",
...
def get_stock_ledger_entries(filters):
	...
	# batch_and_bundle = get_stock_ledger_entries_for_batch_bundle(filters)
	# frappe.throw(f"{entries}")
	return entries
...
def get_stock_ledger_entries_for_batch_no(filters):
	...
	batch_manufacture = frappe.qb.DocType("Batch Manufacture")
	query = (
		frappe.qb.from_(sle)
		.inner_join(batch_manufacture)
		.on(batch_manufacture.name == sle.custom_batch)
		.select(
			sle.item_code,
			sle.warehouse,
			sle.custom_batch,
			sle.posting_date,
			batch_manufacture.expiry_date,
		...
		.where(
			...
			& (sle.custom_batch.isnotnull() & (sle.custom_batch != ""))
		)
		.groupby(sle.voucher_no, sle.custom_batch, sle.item_code, sle.warehouse)
	...
	for field in ["item_code", "custom_batch", "company"]:
...
def get_stock_ledger_entries_for_batch_bundle(filters):
	...
	query = (
		...
		.groupby(sle.voucher_no, sle.custom_batch, batch_package.warehouse)
		.orderby(sle.item_code, sle.warehouse)
	)
...
def get_item_warehouse_batch_map(filters, float_precision):
	...
	for d in sle:
		iwb_map.setdefault(d.item_code, {}).setdefault(d.warehouse, {}).setdefault(
			d.custom_batch, frappe._dict({"opening_qty": 0.0, "in_qty": 0.0, "out_qty": 0.0, "bal_qty": 0.0, "expiry_date": d.expiry_date})
		)
		qty_dict = iwb_map[d.item_code][d.warehouse][d.custom_batch]
