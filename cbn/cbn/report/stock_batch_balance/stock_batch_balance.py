...
def execute(filters=None):
	...
	get_ste_draft(iwb_map, float_precision)
	data = []
					...
					if qty_dict.opening_qty or qty_dict.in_qty or qty_dict.out_qty or qty_dict.bal_qty:
						data.append(
							[
								...
								flt(qty_dict.ste_qty, float_precision),
								flt(qty_dict.net_qty, float_precision),
								", ".join(qty_dict.get("ste_link", [])),
							]
						)
...
def get_columns(filters):
	...
	columns = [
		...
		_("Ste Qty") + ":Float:90",
		_("Net Qty") + ":Float:90",
		_("Ste Draft") + ":Small Text:200",
	]

	return columns

def get_ste_draft(iwb_map, float_precision):
	...
	for d in query:
		...
		if d.s_warehouse:
			qty_dict = item_map.setdefault(d.s_warehouse, {}).setdefault(
				d.batch_no, default_qty_dict()
			)
			qty_dict.ste_qty += -1 * d.actual_qty
			qty_dict.net_qty = flt(qty_dict.bal_qty, float_precision) + flt(qty_dict.ste_qty, float_precision)

			qty_dict.ste_link.append(d.parent)

		if d.t_warehouse:
			qty_dict = item_map.setdefault(d.t_warehouse, {}).setdefault(
				d.batch_no, default_qty_dict()
			)

			qty_dict.ste_qty += d.actual_qty
			qty_dict.net_qty = flt(qty_dict.bal_qty, float_precision) + flt(qty_dict.ste_qty, float_precision)

			qty_dict.ste_link.append(d.parent)

def default_qty_dict():
	return frappe._dict({
		"opening_qty": 0.0, "in_qty": 0.0, "out_qty": 0.0, "bal_qty": 0.0, "ste_qty": 0.0, "net_qty": 0.0, 
		"ste_link": []
	})
